...
const DesktopSidebar = ({ sidebarOpen }) => {
  ...
  return (
    <div className={`peer group fixed left-0 top-0 h-screen w-64 bg-themeCard shadow-themeCard z-10 overflow-hidden border-r border-themeBorder transition-all duration-300 
        ...
        <nav className="md:space-y-1 space-y-3">
          {navItems.map((item) => (
            <a
              ...
              className="flex items-center md:gap-3 gap-3 md:px-0 px-4 md:py-3 py-3 md:pl-5 text-sm font-medium text-themeTextSec hover:text-themeAccent hover:bg-themeAccentBg md:rounded-none rounded-xl transition-all duration-200 whitespace-nowrap md:justify-start"
            >
